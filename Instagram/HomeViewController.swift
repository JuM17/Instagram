...
class HomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    @objc func handleCommentButton(_ sender: UIButton, forEvent event: UIEvent) {
        ...
        // 配列からタップされたインデックスのデータを取り出す
        let postData = postArray[indexPath!.row]
        let ac = UIAlertController(title: "コメント", message: "コメントを入力してください。", preferredStyle: .alert)
        let ok = UIAlertAction(title: "OK", style: .default, handler: {[weak ac] (action) -> Void in
            guard let textField = ac?.textFields?[0] else {
                return
            }
            ...
            if let name = Auth.auth().currentUser?.displayName {
                ...
                let comment = ["name": name, "comment": textField.text!] as [String : String]
